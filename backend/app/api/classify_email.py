...
@classify_router.post(
    "/classify-email", summary="Classify email content using ML model"
)
...
async def classify_email_endpoint(
    request: ClassifyEmailRequestSchema, current_user: dict = Depends(get_current_user)
):
    """
    ...
    result = model_service.classify_text(
        subject=request.subject, body=request.email_body
    )
