...
import { Search, History as HistoryIcon, MailX } from "lucide-react";
...
function HistoryPage() {
  ...
  const loadHistory = useCallback(async (pg: number, labelFilter: string) => {
    setIsLoading(true);
    setError(null);
    try {
                ...
                {filteredEmails.map((email) => (
                  <tr key={email.message_id} className="border-b border-border/40 last:border-0 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="py-3 font-medium pr-2">
                      {truncate(email.subject || "(no subject)", 40)}
                    </td>
                    <td className="py-3 text-muted-foreground pr-2">
                      {truncate(email.snippet || "—", 45)}
                    </td>
