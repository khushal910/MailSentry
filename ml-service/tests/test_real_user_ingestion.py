...
def test_resolve_gmail_label():
    ...
    assert (
        RealUserIngestion.resolve_gmail_label(
            {"gmail_classification": {"is_spam": True}}
        )
        == "spam"
    )
    assert (
        RealUserIngestion.resolve_gmail_label(
            {"gmail_classification": {"is_spam": False}}
        )
        == "ham"
    )
    ...
    assert (
        RealUserIngestion.resolve_gmail_label(
            {"gmail_classification": {"status": "spam"}}
        )
        == "spam"
    )
    assert (
        RealUserIngestion.resolve_gmail_label(
            {"gmail_classification": {"status": "not_spam"}}
        )
        == "ham"
    )
    ...
    assert (
        RealUserIngestion.resolve_gmail_label({"label_ids": ["SPAM", "UNREAD"]})
        == "spam"
    )
...
def test_extract_text_and_subject():
    ...
    subj, msg = RealUserIngestion.extract_text_and_subject(
        {"subject": "Hello", "body": "Full body text", "snippet": "Snippet text"}
    )
    assert subj == "Hello"
    ...
    subj, msg = RealUserIngestion.extract_text_and_subject(
        {"subject": "Hello", "snippet": "Snippet text"}
    )
    assert subj == "Hello"
...
def test_incremental_ingestion_and_idempotency(tmp_path):
    """
    Mandatory Idempotency & Incremental Checkpoint Test.
    Initial: 0 records
    1st Run: 2 emails fetched -> 2 appended
    2nd Run: Same 2 emails fetched (or 0 new _id) -> 0 new appended (count stays 2)
    3rd Run: 1 new email fetched -> 1 new appended (count becomes 3)
    """
    ...
    with patch("src.components.real_user_ingestion.FetchRealUserMail") as MockFetch:
        ...
        mock_fetch_instance.update_last_processed_id.assert_called_with(
            "651a2f000000000000000002", state_file_path=state_file
        )

        # 2nd Run Setup (Same data / no new docs)
        mock_fetch_instance.reset_mock()
        mock_fetch_instance.get_last_processed_id.return_value = (
            "651a2f000000000000000002"
        )
        ...
        # 3rd Run Setup (1 new document)
        mock_docs_batch2 = [
            {
                "_id": "651a2f000000000000000003",
                "message_id": "msg_003",
                "subject": "Ham 2",
        ...
        mock_fetch_instance.update_last_processed_id.assert_called_with(
            "651a2f000000000000000003", state_file_path=state_file
        )
...
def test_fail_safe_checkpoint_protection(tmp_path):
    ...
    with patch("src.components.real_user_ingestion.FetchRealUserMail") as MockFetch:
        ...
        with patch.object(
            pd.DataFrame, "to_csv", side_effect=IOError("Disk write error")
        ):
            ingestion = RealUserIngestion(config=config)
